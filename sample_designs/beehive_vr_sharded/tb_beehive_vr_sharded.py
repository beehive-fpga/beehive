...
from vr_helpers import RequestHdr
...
shard0_machine_config = [
    ("127.0.0.1", 50000),
    ("127.0.0.1", 51000),
    ("127.0.0.1", 52000)
]

shard1_machine_config = [
    ("127.0.0.1", 54000),
    ("127.0.0.1", 55000),
    ("127.0.0.1", 56000)
]

shard2_machine_config = [
    ("127.0.0.1", 40000),
    ("127.0.0.1", 41000),
    ("127.0.0.1", 42000)
]

shard3_machine_config = [
    ("127.0.0.1", 44000),
    ("127.0.0.1", 45000),
    ("127.0.0.1", 46000)
]
...
async def recv_wrapper(tb):
    ...
    pkt = Ether(pkt_bytes)
    tb.log.debug(f"Got packet {pkt.show2(dump=True)}")
    ...
    if (pkt["UDP"].sport == SHARD0_PORT) or (pkt["UDP"].sport == SHARD1_PORT):
...
async def trace_test(tb):
    ...
    shard0_base_pkt = Ether(src="00:00:00:00:00:00", dst="00:00:00:00:00:00",
            type=0x0800)/IP(src="127.0.0.1", dst="127.0.0.1", flags="DF",
                    id=0)/UDP(sport=50000, dport=51000)
    await run_setup(tb, shard0_base_pkt, send_wrapper,
            recv_wrapper, shard0_machine_config, shard0_machine_config[1])
    
    shard1_base_pkt = Ether(src="00:00:00:00:00:00", dst="00:00:00:00:00:00",
            type=0x0800)/IP(src="127.0.0.1", dst="127.0.0.1", flags="DF",
                    id=0)/UDP(sport=54000, dport=55000)

    await run_setup(tb, shard1_base_pkt, send_wrapper, recv_wrapper,
            shard1_machine_config, shard1_machine_config[1])
    
    shard2_base_pkt = Ether(src="00:00:00:00:00:00", dst="00:00:00:00:00:00",
            type=0x0800)/IP(src="127.0.0.1", dst="127.0.0.1", flags="DF",
                    id=0)/UDP(sport=40000, dport=41000)
    await run_setup(tb, shard2_base_pkt, send_wrapper,
            recv_wrapper, shard2_machine_config, shard2_machine_config[1])
    
    shard3_base_pkt = Ether(src="00:00:00:00:00:00", dst="00:00:00:00:00:00",
            type=0x0800)/IP(src="127.0.0.1", dst="127.0.0.1", flags="DF",
                    id=0)/UDP(sport=44000, dport=45000)

    await run_setup(tb, shard3_base_pkt, send_wrapper, recv_wrapper,
            shard3_machine_config, shard3_machine_config[1])

    trace_file = PcapReader(os.environ["BEEHIVE_PROJECT_ROOT"] +
            "/apps/beehive-vr-tile/tb/read_validate_multi_trace.pcap")
    trace_pkt_num = 0
    while True:
        try:
            ...
            if (pkt["UDP"].dport == SHARD0_PORT) or (pkt["UDP"].dport == SHARD1_PORT):
                tb.log.info(f"Sending packet {trace_pkt_num} {pkt.show2(dump=True)}")
                # force rechecksum of the UDP layer since it didn't do it
                await send_wrapper(tb, pkt)
            elif (pkt["UDP"].sport == SHARD0_PORT) or (pkt["UDP"].sport == SHARD1_PORT):
                tb.log.info(f"Receiving packet {trace_pkt_num} {pkt.show2(dump=True)}")
